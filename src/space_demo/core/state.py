...
ESCAPE_ROUTE_DISTANCE_M = 1000.0


@dataclass(frozen=True)
class ProgressHUDState:
    ...
    is_final_phase: bool
    is_complete: bool
    chase_gap_warning: bool


def scaled_wave_duration(raw_duration: float, difficulty: str, is_boss_wave: bool) -> float:
    """Returns wave duration scaled by difficulty. Boss waves are never scaled."""
    if is_boss_wave:
        return raw_duration
    if difficulty == "easy":
        return raw_duration * 0.6
    elif difficulty == "hard":
        return raw_duration * 1.5
    return raw_duration


def raw_wave_duration_from_spec(spec, default: float = 25.0) -> float:
    """Safely extracts raw wave duration from wave specification, supporting default fallback."""
    if spec is None:
        return default
    if hasattr(spec, "duration"):
        return spec.duration
    if hasattr(spec, "get"):
        return spec.get("duration", default)
    return default


def phase_title_for_wave(wave_idx: int, map_def=None) -> str:
    """Returns display title for a given wave index, map-aware and fallback safe."""
    fallbacks = {
        1: "Strategic Retreat 101",
        2: "Throw Interns Overboard",
        3: "Active Cowardice",
        4: "Corporate Audit"
    }
    
    try:
        from space_demo.data.loader import load_flavor_config
        flavor_cfg = load_flavor_config()
        if flavor_cfg is not None and hasattr(flavor_cfg, "get"):
            waves_cfg = flavor_cfg.get("waves", {})
            if waves_cfg is not None and hasattr(waves_cfg, "get"):
                # Resolve from map definition waves if available
                if map_def is not None and hasattr(map_def, "waves") and wave_idx <= len(map_def.waves):
                    wave_key = map_def.waves[wave_idx - 1]
                    if wave_key and hasattr(wave_key, "replace"):
                        title = waves_cfg.get(wave_key) or waves_cfg.get(wave_key.replace("_boss", ""))
                        if title and isinstance(title, str):
                            return title
                            
                # Resolve from general wave_idx format
                title = waves_cfg.get(f"wave_{wave_idx}")
                if title and isinstance(title, str):
                    return title
    except Exception:
        pass
        
    return fallbacks.get(wave_idx, fallbacks.get(4 if wave_idx >= 4 else wave_idx, "Dreadnought Hazard"))


class GameStateManager:
    def _new_chase_pressure_ledger(self) -> dict:
        return {
            "raw_chase_closure_distance": 0.0,
            "pushback_distance_requested_raw": 0.0,
            "pushback_distance_requested_scaled": 0.0,
            "pushback_distance_applied": 0.0,
            "pushback_distance_lost_to_cap": 0.0,
            "pushback_event_count": 0,
            "pushback_by_event_type": {
                "unknown": {"count": 0, "requested_scaled": 0.0, "applied": 0.0, "lost_to_cap": 0.0},
                "boss_hit": {"count": 0, "requested_scaled": 0.0, "applied": 0.0, "lost_to_cap": 0.0},
                "boss_kill": {"count": 0, "requested_scaled": 0.0, "applied": 0.0, "lost_to_cap": 0.0},
                "non_boss_enemy_destruction": {"count": 0, "requested_scaled": 0.0, "applied": 0.0, "lost_to_cap": 0.0},
                "executive_decision": {"count": 0, "requested_scaled": 0.0, "applied": 0.0, "lost_to_cap": 0.0}
            },
            "pushback_by_weapon_type": {
                "unknown": {"count": 0, "requested_scaled": 0.0, "applied": 0.0, "lost_to_cap": 0.0},
                "laser": {"count": 0, "requested_scaled": 0.0, "applied": 0.0, "lost_to_cap": 0.0},
                "missile": {"count": 0, "requested_scaled": 0.0, "applied": 0.0, "lost_to_cap": 0.0},
                "intern_laser": {"count": 0, "requested_scaled": 0.0, "applied": 0.0, "lost_to_cap": 0.0},
                "executive_decision": {"count": 0, "requested_scaled": 0.0, "applied": 0.0, "lost_to_cap": 0.0}
            },
            "lowest_gap_by_wave": {},
            "time_under_100_gap": 0.0,
            "time_under_50_gap": 0.0,
            "time_under_25_gap": 0.0,
            "critical_gap_time": 0.0
        }
    ...
    def __init__(self, ship_def=None, map_def=None, wave_defs=None):
        ...
        self.chase_pressure_ledger = self._new_chase_pressure_ledger()
    ...
    def load_wave_params(self):
        ...
        if self._map_def is not None and self._wave_defs is not None:
            ...
            self.wave_timer = scaled_wave_duration(duration, self.difficulty, is_boss_wave)
            ...
            if self.difficulty == "easy":
                ...
                self.wave_max_enemies = max_enemies
            self.wave_timer = scaled_wave_duration(duration, self.difficulty, self.wave_index == 4)
    ...
    def compute_progress_hud_state(self) -> ProgressHUDState:
        boss_idx = self.boss_wave_index
        
        # Calculate pre-boss retreat durations scaled by difficulty
        wave_durations = []
        if self._map_def is not None:
            waves_list = self._map_def.waves
        else:
            # Fallback wave list
            waves_list = ["wave_1", "wave_2", "wave_3", "wave_4_boss"]
            
        # Consider all pre-boss waves (wave index < boss_wave_index)
        for w_idx in range(1, boss_idx):
            if self._map_def is not None and self._wave_defs is not None:
                if w_idx <= len(waves_list):
                    wave_key = waves_list[w_idx - 1]
                    spec = self._wave_defs.get(wave_key)
                    raw_dur = raw_wave_duration_from_spec(spec, 25.0)
                else:
                    raw_dur = 25.0
            else:
                from space_demo.data.loader import load_waves_config
                waves_cfg = load_waves_config()
                wave_key = waves_list[w_idx - 1] if w_idx <= len(waves_list) else f"wave_{w_idx}"
                spec = waves_cfg.get(wave_key, {"duration": 25.0, "max_enemies": 6})
                raw_dur = spec.get("duration", 25.0)
                
            scaled_dur = scaled_wave_duration(raw_dur, self.difficulty, is_boss_wave=False)
            wave_durations.append(scaled_dur)
            
        total_pre_boss_duration = sum(wave_durations)
        
        # Calculate current elapsed route time and progress fraction
        if self.wave_index >= boss_idx:
            progress_fraction = 1.0
            distance_remaining = 0.0
            is_final_phase = True
        else:
            completed_duration = sum(wave_durations[:self.wave_index - 1])
            # Resolve current wave raw duration
            if self._map_def is not None and self._wave_defs is not None:
                wave_key = waves_list[self.wave_index - 1]
                spec = self._wave_defs.get(wave_key)
                current_raw = raw_wave_duration_from_spec(spec, 25.0)
            else:
                from space_demo.data.loader import load_waves_config
                waves_cfg = load_waves_config()
                wave_key = waves_list[self.wave_index - 1]
                spec = waves_cfg.get(wave_key, {"duration": 25.0, "max_enemies": 6})
                current_raw = spec.get("duration", 25.0)
                
            current_scaled = scaled_wave_duration(current_raw, self.difficulty, is_boss_wave=False)
            current_elapsed = max(0.0, min(current_scaled, self.wave_elapsed_time))
            
            if total_pre_boss_duration > 0.0:
                progress_fraction = max(0.0, min(1.0, (completed_duration + current_elapsed) / total_pre_boss_duration))
            else:
                progress_fraction = 1.0
            ...
            if progress_fraction >= 1.0:
                ...
                distance_remaining = max(0.0, min(ESCAPE_ROUTE_DISTANCE_M, ESCAPE_ROUTE_DISTANCE_M * (1.0 - progress_fraction)))
                
        # Resolve map-aware phase title
        phase_name = phase_title_for_wave(self.wave_index, self._map_def)
        
        # Resolve complete state preferring canonical victory state
        is_boss_defeated = self.boss_active and not any(e.enemy_type == "boss" for e in self.enemies)
        is_complete = (self.current_state == GameStateID.VICTORY) or is_boss_defeated
        
        # Resolve explicit chase gap warning flag
        chase_gap_warning = (self.chase_gap <= config.DREADNOUGHT_CRITICAL_GAP)
        
        return ProgressHUDState(
            progress_fraction=progress_fraction,
            distance_remaining=distance_remaining,
            phase_index=self.wave_index if self.wave_index <= boss_idx else boss_idx,
            phase_name=phase_name,
            is_final_phase=is_final_phase,
            is_complete=is_complete,
            chase_gap_warning=chase_gap_warning
        )

    def effective_capture_gap(self) -> float:
        try:
            from space_demo.playtest.experiments import get_active_overrides
            overrides = get_active_overrides()
            if overrides and overrides.capture_gap is not None:
                return overrides.capture_gap
        except ImportError:
            pass
        if self.difficulty == "medium":
            return config.DREADNOUGHT_MEDIUM_CAPTURE_GAP
        return config.DREADNOUGHT_CAPTURE_GAP
    ...
    def start_game(self):
        ...
        phase_title = phase_title_for_wave(1, self._map_def)
        self.post_event(NotificationEvent(
            title=f"Phase 1: {phase_title}",
            message=f"KEEP DREADNOUGHT GAP ABOVE {self.effective_capture_gap():.0f} METERS!",
            category="phase",
            severity="info"
        ))
    ...
    def update_simulation_tick(self, dt, player_x=0.0, player_y=5.0):
        ...
        self.chase_gap -= closure

        capture_gap = self.effective_capture_gap()
        if self.chase_gap <= capture_gap:
            self.chase_gap = capture_gap
        ...
        # Track threshold exposure and lowest gap in the ledger using the end-of-tick / final chase gap state
        if self.chase_gap < 100.0:
            self.chase_pressure_ledger["time_under_100_gap"] += dt
        if self.chase_gap < 50.0:
            self.chase_pressure_ledger["time_under_50_gap"] += dt
        if self.chase_gap < 25.0:
            self.chase_pressure_ledger["time_under_25_gap"] += dt
        if self.chase_gap < config.DREADNOUGHT_CRITICAL_GAP:
            self.chase_pressure_ledger["critical_gap_time"] += dt

        wave_key = str(self.wave_index)
        if wave_key in self.chase_pressure_ledger["lowest_gap_by_wave"]:
            self.chase_pressure_ledger["lowest_gap_by_wave"][wave_key] = min(
                self.chase_pressure_ledger["lowest_gap_by_wave"][wave_key], self.chase_gap
            )
        else:
            self.chase_pressure_ledger["lowest_gap_by_wave"][wave_key] = self.chase_gap
    ...
    def tick_spawner(self, dt):
        ...
        if self.wave_timer <= 0.0 and self.wave_index < self.boss_wave_index:
            ...
            phase_title = phase_title_for_wave(self.wave_index, self._map_def)
            if self.wave_index == 2:
                self.post_event(NotificationEvent(
                    title=f"Phase 2: {phase_title}",
                ...
                self.post_event(NotificationEvent(
                    title=f"Phase {self.wave_index}: {phase_title} Imminent",
    ...
    def push_back_chaser(self, amount, event_type="unknown", weapon_type="unknown"):
        ...
        if self.boss_active:
            ...
            if amount <= 4.0:
                ...
                scale = 0.60
            requested_scaled = amount * scale
        else:
            requested_scaled = amount

        gap_before = self.chase_gap
        gap_after_unclamped = gap_before + requested_scaled
        gap_after = min(gap_after_unclamped, config.INITIAL_CHASE_GAP)
        applied = gap_after - gap_before
        lost_to_cap = requested_scaled - applied

        self.chase_gap = gap_after

        # Update global totals
        self.chase_pressure_ledger["pushback_distance_requested_raw"] += requested_raw
        self.chase_pressure_ledger["pushback_distance_requested_scaled"] += requested_scaled
        self.chase_pressure_ledger["pushback_distance_applied"] += applied
        self.chase_pressure_ledger["pushback_distance_lost_to_cap"] += lost_to_cap
        self.chase_pressure_ledger["pushback_event_count"] += 1

        # Helper to update sub-dictionaries safely
        def update_sub_dict(sub_dict, key, req_scaled, appl, lost):
            if key not in sub_dict:
                sub_dict[key] = {"count": 0, "requested_scaled": 0.0, "applied": 0.0, "lost_to_cap": 0.0}
            sub_dict[key]["count"] += 1
            sub_dict[key]["requested_scaled"] += req_scaled
            sub_dict[key]["applied"] += appl
            sub_dict[key]["lost_to_cap"] += lost

        update_sub_dict(self.chase_pressure_ledger["pushback_by_event_type"], event_type, requested_scaled, applied, lost_to_cap)
        update_sub_dict(self.chase_pressure_ledger["pushback_by_weapon_type"], weapon_type, requested_scaled, applied, lost_to_cap)
    ...
    def trigger_executive_decision(self, player_x, player_y):
            ...
            if enemy.enemy_type == "boss":
                ...
                if enemy.hp <= 0:
                    if enemy in self.enemies:
                        self.enemies.remove(enemy)
                    self.add_score(100)
                    self.push_back_chaser(config.MISSILE_PUSH_BACK * 0.5, event_type="boss_kill", weapon_type="executive_decision")
        ...
        self.push_back_chaser(4.0, event_type="executive_decision", weapon_type="executive_decision")
        return True
