...
class InputManager:
    ...
    def handle_escape(self):
        ...
        if self.app.state_mgr.current_state == GameStateID.MENU:
            # Exit application if already on start menu
            sys.exit(0)
        else:
            # Return to main menu at any time!
            self.app.state_mgr.transition_to(GameStateID.MENU)
            if hasattr(self.app, "screens") and self.app.screens:
                self.app.screens.menu_sub_state = "main"
            self.app.clear_views()
            
            # Switch back to menu ambient track
            if not self.headless:
                if self.app.audio_mgr.chase_music:
    ...
    def handle_resolution(self, width, height):
        has_settings = hasattr(self.app, 'screens') and self.app.screens and getattr(self.app.screens, 'menu_sub_state', None) == "settings"
        if self.app.state_mgr.current_state == GameStateID.MENU and has_settings:
            if not self.headless:
                from panda3d.core import WindowProperties
                props = WindowProperties()
                props.setSize(width, height)
                
                # Center window on screen using pipe desktop metrics
                w = self.app.pipe.getDisplayWidth()
                h = self.app.pipe.getDisplayHeight()
                if w > 0 and h > 0:
                    props.setOrigin((w - width) // 2, (h - height) // 2)
                    
                self.app.win.requestProperties(props)
                print(f"[Window] Resolution changed: {width}x{height} (centered)")
                self._sync_cursor()

    def toggle_tactical_console(self):
        """Collapse/expand the left tactical console without affecting gameplay."""
        if self.app.state_mgr.current_state == GameStateID.PLAYING and hasattr(self.app, "hud"):
            self.app.hud.toggle_panel("tactical_console")

    def toggle_tactical_log(self):
        """Collapse/expand the tactical log panel without affecting gameplay."""
        if self.app.state_mgr.current_state == GameStateID.PLAYING and hasattr(self.app, "hud"):
            self.app.hud.toggle_panel("tactical_log")

    def toggle_pursuit_gauge(self):
        """Collapse/expand the right pursuit gauge without affecting gameplay."""
        if self.app.state_mgr.current_state == GameStateID.PLAYING and hasattr(self.app, "hud"):
            self.app.hud.toggle_panel("pursuit_gauge")

    def toggle_pause(self):
        curr = self.app.state_mgr.current_state
        if curr == GameStateID.PLAYING:
            self.app.state_mgr.transition_to(GameStateID.PAUSED)
            if hasattr(self.app, "screens") and self.app.screens:
                self.app.screens.update_screen_state(GameStateID.PAUSED)
            if not self.headless:
                self.app.set_cursor_visible(True)
                self._sync_cursor()
        elif curr == GameStateID.PAUSED:
            self.app.state_mgr.transition_to(GameStateID.PLAYING)
            if hasattr(self.app, "screens") and self.app.screens:
                self.app.screens.update_screen_state(GameStateID.PLAYING)
            if not self.headless:
