...
from space_demo.startup_branding import GAME_WINDOW_TITLE, apply_startup_branding, data_dir
...
class SpaceDisasterApp(ShowBase):
    ...
    def __init__(self, headless=False, profile_path=None, default_profile_path=None, player_ships_path=None, ship_adapter=None):
        ...
        self.early_splash_np = None
        self._early_splash_bg_np = None
        ...
        if self.headless:
            ...
            self._apply_runtime_window_branding()
            self._show_early_startup_splash()
        ...
        if not self.headless:
            ...
            self._hide_early_startup_splash()
    ...
    def _apply_runtime_window_branding(self) -> None:
        """Apply window title and icon defensively and platform-safely."""
        if self.headless or not self.win:
            return

        from panda3d.core import WindowProperties, Filename
        props = WindowProperties()
        props.setTitle(GAME_WINDOW_TITLE)

        try:
            icon_ico = data_dir() / "game_icon.ico"
            icon_png = data_dir() / "game_icon.png"
            icon_path = icon_ico if icon_ico.exists() else icon_png
            
            if icon_path.exists():
                props.setIconFilename(Filename.fromOsSpecific(str(icon_path)))
        except Exception as exc:
            if not self.headless:
                print(f"[Branding Warning] Failed to configure window icon: {exc}")
        ...
        try:
            ...
            if not self.headless:
                print(f"[Branding Warning] Failed to apply window properties: {exc}")

    def _show_early_startup_splash(self) -> None:
        """Create and display an early, undistorted, full-window loading splash overlay."""
        if self.headless:
            return

        try:
            splash_path = data_dir() / "startup_splash.png"
            if splash_path.exists():
                tex = self.loader.loadTexture(Filename.fromOsSpecific(str(splash_path)))
                
                # 1. Background fill card parented to render2d to cover full viewport margins
                bg_cm = CardMaker("early_splash_bg")
                bg_cm.setFrame(-1.0, 1.0, -1.0, 1.0)
                self._early_splash_bg_np = self.render2d.attachNewNode(bg_cm.generate())
                self._early_splash_bg_np.setColor(8/255.0, 16/255.0, 32/255.0, 1.0)
                self._early_splash_bg_np.setBin("fixed", 9999)
                self._early_splash_bg_np.setDepthWrite(False)
                self._early_splash_bg_np.setDepthTest(False)
                
                # 2. Centered aspect-ratio safe splash card parented to aspect2d
                splash_cm = CardMaker("early_splash_logo")
                splash_cm.setFrame(-16.0/9.0, 16.0/9.0, -1.0, 1.0)
                self.early_splash_np = self.aspect2d.attachNewNode(splash_cm.generate())
                self.early_splash_np.setTexture(tex)
                self.early_splash_np.setBin("fixed", 10000)
                self.early_splash_np.setDepthWrite(False)
                self.early_splash_np.setDepthTest(False)

                # Force frames to render immediately so the splash is visible before asset loading
                self.graphicsEngine.renderFrame()
                self.graphicsEngine.renderFrame()
        except Exception as exc:
            print(f"[Branding Warning] Failed to display early startup splash: {exc}")

    def _hide_early_startup_splash(self) -> None:
        """Remove the early startup splash and background card idempotently."""
        if hasattr(self, "early_splash_np") and self.early_splash_np is not None:
            try:
                self.early_splash_np.removeNode()
            except Exception:
                pass
            self.early_splash_np = None

        if hasattr(self, "_early_splash_bg_np") and self._early_splash_bg_np is not None:
            try:
                self._early_splash_bg_np.removeNode()
            except Exception:
                pass
            self._early_splash_bg_np = None
