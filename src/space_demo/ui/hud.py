...
from space_demo.ui.components import GameButton, GameMeter, GamePanel, wrap_text_lines
...
class GameHUD:
    ...
    def _build_pursuit_gauge(self) -> None:
        ...
        self.pursuit_panel = GamePanel(
            ...
            pos=(-0.14 * self.ui_scale, 0, -0.02 * self.ui_scale),
            size=(0.18 * self.ui_scale, 0.66 * self.ui_scale),
            bg_color=COLORS.panel_bg_dark,
            stroke_color=COLORS.amber,
            border_thickness=0.005 * self.ui_scale,
        )
        self.pursuit_toggle_expanded_pos = (-0.253 * self.ui_scale, 0, 0.290 * self.ui_scale)
        self.pursuit_toggle_collapsed_pos = (-0.023 * self.ui_scale, 0, 0.290 * self.ui_scale)
        ...
        self.pursuit_track = DirectFrame(
            parent=root,
            frameSize=(-0.042 * self.ui_scale, 0.042 * self.ui_scale, -0.225 * self.ui_scale, 0.225 * self.ui_scale),
            pos=(0, 0, 0.020 * self.ui_scale),
        ...
        self.proximity_label = self._label(root, "DREADNOUGHT GAP\n200.0 m", (0, 0, -0.260 * self.ui_scale), 0.0105)
        self.prox_panel = self.pursuit_panel
        self.distance_bar_bg = DirectFrame(parent=root, frameSize=(0, 0, 0, 0))
        self.distance_bar = DirectFrame(parent=root, frameSize=(0, 0, 0, 0))
    ...
    def update(self, state_mgr, player, dt: float = 0.0) -> None:
        ...
        self.hull_label["text"] = f"REAR HULL: {state_mgr.player_hp}/{config.PLAYER_START_HP}"
        ...
        self.score_label["text"] = f"SYNERGY VALUE: {state_mgr.score}"
        self.multiplier_label["text"] = f"SYNERGY MULT: {state_mgr.synergy_multiplier:.1f}x"
        self.multiplier_label["text_fg"] = COLORS.text_warning if state_mgr.synergy_multiplier > 1.0 else COLORS.text_primary
        self.missile_label["text"] = f"MISSILES: {state_mgr.missile_ammo}/{config.MAX_MISSILES}"
        self._set_fraction_bar(self.missile_bar, state_mgr.missile_ammo / config.MAX_MISSILES, COLORS.red)
        self.bomb_label["text"] = f"DECISIONS: {state_mgr.bomb_ammo}/3"
        self._set_fraction_bar(self.bomb_bar, state_mgr.bomb_ammo / 3, COLORS.orange)

        if state_mgr.magnet_active_timer > 0.0:
            self.magnet_pill["text"] = f"MAGNET: {state_mgr.magnet_active_timer:.1f}s"
            self.magnet_pill.show()
        else:
            self.magnet_pill.hide()
        if state_mgr.intern_active_timer > 0.0:
            self.intern_pill["text"] = f"INTERN: {state_mgr.intern_active_timer:.1f}s"
            self.intern_pill.show()
        else:
            self.intern_pill.hide()

        wave_titles = {1: "Strategic Retreat 101", 2: "Throw Interns Overboard", 3: "Active Cowardice", 4: "Corporate Audit"}
        self.wave_pill["text"] = f"Phase {state_mgr.wave_index}: {wave_titles.get(state_mgr.wave_index, 'Dreadnought Hazard')}"
        if state_mgr.boss_active:
        ...
        bottom_z = -0.190 * self.ui_scale
        top_z = 0.205 * self.ui_scale
        ...
        if gap <= config.DREADNOUGHT_CRITICAL_GAP:
            ...
            self.proximity_label["text"] = f"CRITICAL GAP\n{gap:.1f} m"
            ...
            self.proximity_label["text"] = f"DREADNOUGHT GAP\n{gap:.1f} m"
            self.dread_icon.setScale(1.0)

        if state_mgr.active_bark:
            self.bark_label["text"] = state_mgr.active_bark
            self.bark_label.show()
