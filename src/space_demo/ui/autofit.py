...
@dataclass(frozen=True)
class TextFitPolicy:
    """Shared text fitting policy for existing DirectGUI widgets."""
...
def fit_direct_text_to_policy(
    ...
    if scale is not None:
        if line_count > policy.max_lines:
            scale *= policy.max_lines / max(line_count, 1)
        if longest_line > policy.max_chars_per_line:
            scale *= policy.max_chars_per_line / max(longest_line, 1)
...
def apply_screen_widget_autofit(screen_owner: Any, policy: TextFitPolicy = DEFAULT_TEXT_FIT_POLICY) -> None:
    ...
    for widget in iter_screen_widgets(screen_owner):
        fit_directgui_widget_text(widget, policy)
