...
from space_demo.ui.pause_ux import PAUSE_HELP_TEXT, apply_pause_overlay_copy
...
class GameMenuScreens:
    ...
    def _build_pause(self) -> None:
        ...
        self.paused_slogan = self._label(
            ...
            f"Administrative hold active.\n{PAUSE_HELP_TEXT}",
            (0, 0, -0.05),
            0.030,
            COLORS.text_primary,
        )

    def refresh_pause_overlay(self) -> None:
        """Refresh pause overlay elements visibility and styling properties."""
        apply_pause_overlay_copy(self)
        
        if hasattr(self, "paused_title") and self.paused_title is not None:
            try:
                self.paused_title.show()
            except Exception:
                pass
            
        if hasattr(self, "paused_slogan") and self.paused_slogan is not None:
            try:
                self.paused_slogan.setScale(0.025 * self._effective_text_scale())
                self.paused_slogan.setPos(0, 0, -0.05)
                self.paused_slogan["text_pos"] = (0, 0)
                self.paused_slogan["text_fg"] = COLORS.text_primary
                self.paused_slogan.show()
            except Exception:
                pass
    ...
    def refresh_settings_controls_visuals(self) -> None:
        ...
        update_label_text(self.res_label, f"RESOLUTION: {width} x {height}")
        ...
        update_label_text(self.ui_scale_label, f"UI SCALE: {int(self.pending_ui_scale * 100)}%")
        ...
        update_label_text(self.text_scale_label, f"TEXT SIZE: {int(self.pending_text_scale * 100)}%")
        ...
        update_label_text(self.difficulty_label, f"DIFFICULTY: {self.pending_difficulty.upper()}")
        ...
        update_label_text(self.btn_hud_density_label, f"HUD DENSITY: {HUD_DENSITY_LABELS[self.pending_hud_density]}")
    ...
    def on_master_vol_change(self) -> None:
        ...
        update_label_text(self.master_vol_label, f"MASTER VOLUME: {int(self.pending_master_volume * 100)}%")
    ...
    def on_music_vol_change(self) -> None:
        ...
        update_label_text(self.music_vol_label, f"MUSIC VOLUME: {int(self.pending_music_volume * 100)}%")
    ...
    def on_sfx_vol_change(self) -> None:
        ...
        update_label_text(self.sfx_vol_label, f"SFX VOLUME: {int(self.pending_sfx_volume * 100)}%")
